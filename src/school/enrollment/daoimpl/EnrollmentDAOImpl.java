...
public class EnrollmentDAOImpl implements EnrollmentDAO {
    ...
    public List<Enrollment> getAll() throws Exception {
        ...
        String sql = "SELECT e.*, s.first_name, s.last_name, c.course_code, c.course_name, c.units, c.tuition_per_unit " +
                     ...
                     "JOIN students s ON e.student_id = s.student_id " +
                     "JOIN courses c ON e.course_id = c.course_id " +
                      "WHERE e.status='Enrolled' AND s.active=1 ORDER BY e.enrollment_date DESC";
        try (Connection conn = DatabaseConnection.getConnection();
    ...
    public List<Enrollment> getByStudent(String studentId) throws Exception {
        ...
        String sql = "SELECT e.*, s.first_name, s.last_name, c.course_code, c.course_name, c.units, c.tuition_per_unit " +
                     ...
                     "JOIN students s ON e.student_id = s.student_id " +
                     "JOIN courses c ON e.course_id = c.course_id " +
                     "WHERE e.student_id = ? AND e.status='Enrolled' AND s.active=1 ORDER BY e.enrollment_date DESC";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public List<Enrollment> getByCourse(int courseId) throws Exception {
        ...
        String sql = "SELECT e.*, s.first_name, s.last_name, c.course_code, c.course_name, c.units, c.tuition_per_unit " +
                     ...
                     "JOIN students s ON e.student_id = s.student_id " +
                     "JOIN courses c ON e.course_id = c.course_id " +
                     "WHERE e.course_id = ? AND e.status='Enrolled' AND s.active=1 ORDER BY e.enrollment_date DESC";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public List<Enrollment> search(String keyword) throws Exception {
        ...
        String sql = "SELECT e.*, s.first_name, s.last_name, c.course_code, c.course_name, c.units, c.tuition_per_unit " +
                     ...
                     "JOIN students s ON e.student_id = s.student_id " +
                     "JOIN courses c ON e.course_id = c.course_id " +
                     "WHERE e.status='Enrolled' AND s.active=1 AND (s.first_name LIKE ? OR s.last_name LIKE ? OR c.course_name LIKE ? OR c.course_code LIKE ?) " +
